...
## I. CẤU TRÚC - MAPPING
> Nếu không muốn tự cài có thể import file OVA, link tải ở cuối
### 1. Yêu cầu:
- RAM: 8Gb
- CPU: 4
...
- **Mount tự động**
    ...
    sudo nano /etc/fstab
    ```
    - Thực hiện thêm UUID vào, đóng và lưu file lại
    ```bash
    # cú pháp: 
...
#### Kiểm tra trạng thái của contrainer đảm bảo `up (healthy)`
```bash
sudo docker ps -a
```
```bash
...
55464e505ff3   zabbix/zabbix-web-nginx-pgsql:ubuntu-latest   "docker-entrypoint.sh"   5 minutes ago   Up 5 minutes (healthy)   0.0.0.0:80->8080/tcp, [::]:80->8080/tcp, 0.0.0.0:443->8443/tcp, [::]:443->8443/tcp   zabbix-web
87585f78bdf6   zabbix/zabbix-server-pgsql:ubuntu-latest      "/usr/bin/docker-ent…"   5 minutes ago   Up 5 minutes (healthy)   0.0.0.0:10051->10051/tcp, [::]:10051->10051/tcp                                      zabbix-server
548fc79f5fb6   postgres:15-alpine                            "docker-entrypoint.s…"   5 minutes ago   Up 5 minutes (healthy)   5432/tcp                                                                             zabbix-postgres
```
...
### 4. Nếu KHÔNG muốn cài, có thể import file OVA vào VMWare (Ver 16.2.1 1881642 - VMWare Pro)
- Link tải: https://drive.google.com/file/d/1d6obOZ2z0a9lLLgmIOK12fykQuwLECOk/view?usp=drive_link
- Thông tin login sau khi import thành công (zabbix 7.4):
